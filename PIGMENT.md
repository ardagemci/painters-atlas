...
| D-8 | **The sealed-set hook** — `CLAUDE.md` §0 states that a Lane III run's inability to write the sealed set is "enforced by hook rather than by instruction" | `CLAUDE.md` §0, added 2026-08-17 | **Built, unit-proven, never fired in anger.** `.claude/hooks/sealed-set.py` exists and is registered in `.claude/settings.json` on `Write\|Edit\|MultiEdit\|NotebookEdit\|Bash`; 29 tests in `tests/test_lane3.py` cover it. It arms only on `PIGMENT_LANE3=1`, which only the runner sets, so Lane II is untouched. What is **not** yet true: it has never been exercised by a live Claude Code session, because that requires a Lane III run, which requires the CLI this Mac does not have. A hook that passes its own tests and has never met a real harness is evidence, not proof. | Phase 2's first live dry run. The grant precondition in `protocol/writs/README.md` is therefore an **observed** refusal, not a present file — a distinction this row exists to keep honest. |
