...
| D-8 | **The sealed-set hook** — `CLAUDE.md` §0 states that a Lane III run's inability to write `tools/validate*`, `tools/audit_*.py`, `CLAUDE.md`, `PIGMENT.md`, `protocol/` or `.claude/` is "enforced by hook rather than by instruction" | `CLAUDE.md` §0, added 2026-08-17 | Not implemented. No `PreToolUse` hook exists; `.claude/settings.local.json` carries only a permissions allow-list. The sentence describes the control the lane is designed around, and today that control is prose. Consequence is currently nil — no writ is granted (`W-001` is `proposed`) and no unattended run exists — but the constitution is ahead of the build, and the whole design rests on this one guarantee. | Phase 3 of the build-lane work, which must complete **before** any writ is granted. Recorded here rather than softened because the sentence states the correct target: an agent that can edit its own grader can make any change pass, and instruction alone does not stop that. |
