...
Five axes, each scored **−100 … +100** (canonical scale; all engine math lives in `TASTE_MATH.md`). The fifth is stored from day one but may stay hidden in UI at launch.
...
| Movement | F | D | E | C | M |
|---|---|---|---|---|---|
| Impressionism | −50 | −40 | +10 | −70 | −30 |
| Baroque | −90 | +80 | −60 | −30 | +50 |
| Suprematism | +100 | −10 | +90 | +70 | −10 |
| Dutch Golden Age | −90 | −50 | −50 | −40 | −80 |
| Surrealism | −30 | +40 | +60 | +40 | −10 |
| Pop Art | −40 | +10 | +60 | +80 | +10 |

Storage: `coords` field on movement/artist/artwork objects; the JXA validator checks presence (Tier 1) and range (all).

## 5. User taste vector

**Position = shrunken centroid; shape = 1-or-2 component mixture.** Full equations, worked examples and the cluster-split algorithm live in **`TASTE_MATH.md`** — the summary contract:

```
userVector = ( Σ admiredArtwork.coords · w  +  k · quizPrior ) / ( Σ w + k )
```

- `w = 1.0` for all real-artwork admirations (no signal-source weighting in v1); `w = 0.5` for weak-signal probes (§7).
- `quizPrior` from §6.3; `k = 6` — the quiz "weighs" six artworks and **decays automatically** as admirations grow (never a fixed 70/30 blend).
- **Mixture in 1.5, not later:** a deterministic 2-means split (accepted only when it explains ≥40% of spread; min 3 items per island; ≥50 apart in F×D) prevents the day-one "Vermeer + Kandinsky = beige centroid" failure and powers hybrid Personas.
- No recency decay in v1 (`decay` reserved).
- **Confidence is per-axis** internally (posterior variance + coverage); the count-based tiers — provisional (onboarding) → forming (20) → solid (50) → strong (100) → deep (250+) — are the *copy layer*.
- **Dispersion matters:** high spread with a rejected split is not noise — it's eclecticism, and it feeds General Personas (§8).
...
- ≥3 works with `F ≥ +30` (real abstraction exists pre-1956: Kandinsky, Malevich, Popova, Klee, af Klint)
...
Each answer applies nudges of ±30 (±15 for soft options) to 1–2 axes, summed and capped at ±60 per axis:
...
- **Specific Persona** = prototype vector + optional signature condition (e.g., *[Dutch-interior persona]* ≈ `{−80, −50, −40, −40, −80}` + admired ≥2 Dutch Golden Age works). Matching runs **per taste island** — two accepted mixture components yield a primary and secondary Persona.
...
```json
{
  ...
  "skipped":    [ "the-birth-of-venus-bouguereau" ],
  "notForMe":   [],
  "seen":       [ { "id": "mona-lisa", "at": "…", "museumId": null } ],
  "wantToSee":  [ "las-meninas" ],
  "saved":      [ "the-tortoise-trainer" ],
  "deckSeen":   [ "girl-with-a-pearl-earring", "saturn-devouring-his-son" ],
  "probes":     [ { "kind": "movement", "id": "pop-art", "at": "…" } ],
  "quiz":       { "answers": { "q1": "b" }, "prior": { "F": 0, "D": 60, "E": 0, "C": 30, "M": 0 }, "at": "…" },
  "palette":    { "tones": ["ultramarine", "caravaggio-black", "museum-cream", "dried-rose"], "source": "chosen" },
  "persona":    { "adopted": null, "candidates": [], "adoptedAt": null, "hidden": false, "provisional": true },
  "tasteVector": { "F": -34, "D": 53, "E": -14, "C": 9, "M": -4, "n": 14, "sd": 18 },
  "clusters":   null,
  "milestones": { "onboarded": true, "confidence": "forming" }
}
```

Field notes: `skipped` = onboarding silence (never negative in v1); `saved` = *save for later* (interest without admiration — distinct from `wantToSee`, which is the physical wishlist); `deckSeen` tracks onboarding exposure; `clusters` holds the accepted 2-component mixture when the split fires (`[{center, weight, n}, …]`); `persona.provisional` stays true until confidence and quiz-evidence agreement allow otherwise.
