...
- **Date / round / state:** 2026-08-05 / round 3 / revision (unit 35)
- **What changed:** Four things, all conceding findings rather than defending positions.
  **(1) OD-5 language sweep, 14 sites.** The build evidence report's two phrases asserting a legal status for replacement images were struck and restated as what the evidence supports: Commons metadata *asserts* a public-domain basis, the exact-work check *confirmed* the depicted work, and no independent legal determination was made in any jurisdiction. Twelve further sites were corrected across `evidence/rights-register.md`, `evidence/rights-remediation.md`, `evidence/build-log-unit-24.md`, `tools/fetch_artworks.py`, `tools/audit_artworks.py`, `tools/build_photo_credits.py`, `docs/STYLE_GUIDE.md` and `README.md`, including two exhaustive-absence claims about Kahlo images on Commons and two jurisdictional conclusions about term of protection. `js/photo-credits.js` was regenerated from the corrected template; **its credit data is byte-identical** and only the header language changed. `docs/ARTWORK_SCHEMA.md` now records that `status:"pd"` is a rendering token, not a legal finding.
  **(2) The rule is now enforced where it actually broke.** `tests/test_rights_tooling.py` gains `TestProseLanguage`, which reads this pole's prose artifacts and fails on ten patterns of legal assertion, with a negative fixture (the six real strings that got through), a positive fixture (bounded wording must not be flagged), and a pinned exemption map so that widening the exemption is itself a test failure.
  **(3) One denominator reconciliation** published at `evidence/data-reconciliation.md`, binding every current and historical rights and inventory figure to its value, commit, date, surface and exact meaning; and the AC11 **no-asset disposition** for `beginning-noland`, the 76th Tier 1 record.
  **(4) The test suite passes** — 41 tests with 5 failures becomes 46 tests with 0 — with every changed expectation carrying a recorded reason, and the latent sixth failure (`copyright_refs` 60) fixed in the same pass.
- **Why it changed:** The theory pole's round-3 review is correct on the language finding, and it is a breach of the owner's own decision (OD-5, 2026-07-23), not merely of their preference. The correct posture was to concede without negotiation.
- **Why the *shape* of the breach matters:** `TestRegisterLanguage` enforces exactly this restraint on the machine-generated register and has always passed. The rule broke only in human-written prose, because nothing read the prose. **A rule enforced only where a test looks is not enforced** — which is why the remedy is a test and not only a correction. The new guard immediately earned itself: it caught five breaches a hand-written grep had missed, one of them in a file this unit had already corrected once, and one in this unit's own first draft of a correction.
- **Corrections to the record, made against interest and against both poles:**
  - **Ours.** The report cited 694 renderable images in the present tense; 694 was the 2026-07-24 census figure and the current value is 693. An error of tense, not of census.
  - **Theirs (theory).** The 76-vs-75 Tier 1 finding has the right remedy and a wrong diagnosis: `beginning-noland` carries `image:{status:"copyright"}` with no `src`, so a register built from records bearing a Commons URL correctly counts 75, and the assertion encoding that 75 passes and was never among the five failures. What was owed was the no-asset disposition, now given.
  - **Theirs (theory).** `66`-vs-`60` was listed among the reproduced failures; it was latent, not live — the assertion was unreached because an earlier line in the same method aborted first. Fixed together with that line, so no sixth failure surfaces.
  - **Both poles.** The added museum photograph is **Hirshhorn**, not Guggenheim. Guggenheim is present in the `effa805` freeze and did not change. The misidentification came from reading `unittest`'s "first extra element" — a positional artefact of comparing sorted lists — as the set difference.
  - **New measurement neither pole had.** At `effa805` the catalog∩gallery overlap was 91 and artwork-only was 695, giving the full chain 695 (`effa805`) → 694 (census) → 693 (now). Separately, **695 is a genuine value collision**: it is both the `effa805` artwork-only asset count and the current stub-*file* count.
- **Assumption or constraint that required it:** OD-5 and AC12 (no clearance claim, ever); AC10's requirement that inventory counts be exact and re-verifiable by surface; the instruction that a green suite bought by loosening an assertion is worse than a red one.
- **Supporting evidence:** `evidence/data-reconciliation.md`; `evidence/asset-inventory-a1b822b.{json,md}` regenerated at `a1b822b` with the `effa805` freeze left byte-stable; `python3 -m unittest discover -s tests` → **Ran 46 tests, OK**; `osascript -l JavaScript tools/validate.jxa.js` → `ALL REFERENCES VALID`, **zero warnings**; `asset_inventory.py --compare` → 4 drifted surfaces, every `+`/`−` line absorbed by `CORRECTIONS` or `CONTENT_LANE`.
- **Effect on user experience:** None to the interface. To the reader of the evidence: every rights statement is now bounded to what was observed, and every denominator can be checked against a named commit and surface.
- **Status:** accepted. **One item deliberately left open and escalated rather than fixed:** `js/app.js:2393` ships the credits-page lede "Most reproductions here are public domain", a bare legal assertion on a production surface. This unit does not own `js/app.js` — it is frozen behind another unit's contrast and screenshot evidence — so the line was reported, not edited. `TestProseLanguage` therefore does not yet scan `js/`; whoever next owns that file should correct the lede and add `js/` to the guard's scope.
- **Raised by / decided by:** chatgpt-theory (round 3 review, requested actions 4–7) / Seurat (Data and Copyright Steward), unit 35.
